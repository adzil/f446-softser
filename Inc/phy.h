...
#if defined(DRV_TEST)
#define PHY_PAYLOAD_ENC_LEN(LEN) FEC_CC_BUFFER_LEN(FEC_RS_BUFFER_LEN(LEN))
#define PHY_PAYLOAD_DEC_LEN(LEN) PHY_PAYLOAD_ENC_LEN(LEN)
#elif defined(MAC_COORDINATOR)
#define PHY_PAYLOAD_ENC_LEN(LEN) FEC_CC_BUFFER_LEN(FEC_RS_BUFFER_LEN(LEN))
#define PHY_PAYLOAD_DEC_LEN(LEN) FEC_RS_BUFFER_LEN(LEN)
#else
#define PHY_PAYLOAD_ENC_LEN(LEN) FEC_RS_BUFFER_LEN(LEN)
#define PHY_PAYLOAD_DEC_LEN(LEN) FEC_CC_BUFFER_LEN(FEC_RS_BUFFER_LEN(LEN))
#endif
#define PHY_HEADER_ENC_LEN PHY_PAYLOAD_ENC_LEN(PHY_HEADER_LEN)
#define PHY_HEADER_DEC_LEN PHY_PAYLOAD_DEC_LEN(PHY_HEADER_LEN)
#define PHY_PAYLOAD_MAX_ENC_LEN PHY_PAYLOAD_ENC_LEN(MAC_CONFIG_MAX_FRAME_BUFFER)
#define PHY_PAYLOAD_MAX_DEC_LEN PHY_PAYLOAD_DEC_LEN(MAC_CONFIG_MAX_FRAME_BUFFER)

#ifdef DRV_TEST
extern uint8_t TestRcv[DRV_TEST];
extern osThreadId tid_DrvTest;
#endif
